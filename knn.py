...
def compute_distances(Xtrain, X):
	...
	m, dimensions= X.shape
	n, dimensions= Xtrain.shape
	size= (m, n)
	dists= np.zeros(size)

	for i in range(0,m):
		diff= np.subtract(X[i], Xtrain)
		diff= diff ** 2
...
def predict_labels(k, ytrain, dists):
	...
	idx_classes= np.argsort(dists, axis=1)[ : , :k]	# Gets the indexes of top k min values from all the rows
	yprobables= ytrain[idx_classes]
	m, n= yprobables.shape
	classes= np.amax(ytrain)+1
	counts= np.zeros([m, classes], dtype=int)
	
	for i in range(0, m):
		for j in range(0, k):
			counts[i][yprobables[i][j]]+= 1
