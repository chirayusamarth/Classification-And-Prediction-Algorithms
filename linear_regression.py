...
def linear_regression_noreg(X, y):
  ...
  inverseIntoXtranspose= np.matmul(inverse, Xtranspose)
  w= np.matmul(inverseIntoXtranspose, y)
...
def regularized_linear_regression(X, y, lambd):
  ...
  Xtranspose= X.transpose()
  XtransposeX= np.matmul(Xtranspose, X)
  m, n= XtransposeX.shape
  I= np.identity(m)
  lambdI= lambd * I
  addition= np.add(XtransposeX, lambdI)
  inverse = np.linalg.inv(addition)
  inverseIntoXtranspose= np.matmul(inverse, Xtranspose)
  w= np.matmul(inverseIntoXtranspose, y)
...
def test_error(w, X, y):
  ...
  err= 0
  for i in range(0, m):
  	wTx= np.matmul(wTranspose,X[i])
  	diff= np.subtract(y[i], wTx)
  	diff= diff ** 2
  	err+= diff
